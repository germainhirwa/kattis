...
public class MuddyHike {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int[][] muds = new int[r][c]; // 2D array to store the height of the muds, we will determine the graph weight from here
        ...
        AdjacencyList graph = new AdjacencyList(r*c+1,muds);
        for (int i = 0; i < r; i++) {
            for (int j = 0; j < c; j++) {
                // Check neighbouring muds and then connect them
                if (j > 0)
                    graph.connect(i*c+j,i*c+(j-1),muds[i][j-1]); // left
                if (j < c-1)
                    graph.connect(i*c+j,i*c+(j+1),muds[i][j+1]); // right
                if (i > 0)
                    graph.connect(i*c+j,(i-1)*c+j,muds[i-1][j]); // above
                if (i < r-1)
                    graph.connect(i*c+j,(i+1)*c+j,muds[i+1][j]); // below
            }
        }
        for (int i = 0; i < r; i++)
            graph.connect(r*c, i*c, muds[i][0]);
...
class AdjacencyList { // D/W graph DS
    ...
    public AdjacencyList (int V, int[][] arr) {
        ...
        muds = arr; // store the 2D array here for easy access
    ...
    public int MST() { // we will only run this once, hence initialization of taken boolean and parent int array is at the start of AL construction
        List<List<Pair>> mst = new ArrayList<List<Pair>>();
        int rows = muds.length;
        int cols = muds[0].length;
        int s = rows*cols;
        ...
        for (int i = 0; i < rows; i++) {
            int temp = 0;
            int curr = cols*i + cols - 1;
            while (curr != rows*cols) {
                temp = Math.max(temp, muds[curr / cols][curr % cols]);
                curr = parent[curr];
            }
