...
for _ in range(int(input())):
    p = []; a = 0
    for _ in range(int(input())):
        x, y, w, h, v = map(float, input().split()); v *= pi/180
        vecs = [(-w/2, h/2), (w/2, h/2), (-w/2, -h/2), (w/2, -h/2)]
        for dx, dy in vecs:
            ddx = dx*cos(v)+dy*sin(v)
            ddy = -dx*sin(v)+dy*cos(v)
            p.append((x+ddx, y+ddy))
        a += w*h
    print('%.1f'%(a/area(chull(p))*100), '%')