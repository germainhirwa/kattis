...
for l in sys.stdin:
    ...
    if l[0] == '+1':
        mol.append(Counter())
        for i in range(1, len(l)//2): mol[-1][l[2*i]] += int(l[2*i+1]); el.add(l[2*i])
    elif l[0] == '-1':
        mol.append(Counter())
        for i in range(1, len(l)//2): mol[-1][l[2*i]] -= int(l[2*i+1]); el.add(l[2*i])
rev = {e:i for i,e in enumerate(el)}
mat = [[Fraction()]*len(mol) for _ in range(len(el))]
for i in range(len(mol)):
    for k, v in mol[i].items(): mat[rev[k]][i] = Fraction(v, 1)
for i in range(len(el)): mat[i][-1] = -mat[i][-1]
...
ans = [row[-1] for row in solve(mat) if row[-1] != 0]; mult = 1
for f in ans: mult = lcm(mult, f.denominator)
print(*(f.numerator for f in [f*mult for f in ans]), mult)