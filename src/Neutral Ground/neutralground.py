...
for i in range(r):
    for j in range(c):
        if m[i][j] == 'A':      add_edge(V-2, 2*(i*c+j), 10**9), add_edge(2*(i*c+j), 2*(i*c+j)+1, 10**9, 1)
        elif m[i][j] == 'B':    add_edge(2*(i*c+j)+1, V-1, 10**9), add_edge(2*(i*c+j), 2*(i*c+j)+1, 10**9, 1)
        else:                   add_edge(2*(i*c+j), 2*(i*c+j)+1, int(m[i][j]), 1)
        for di, dj in delta:
